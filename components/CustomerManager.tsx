...
export const CustomerManager: React.FC = () => {
  ...
  const openCustomerDetails = async (customer: Customer) => {
    ...
    const loans = await db.loans.getByCustomerId(customer.id);
  ...
  const handleAddPayment = async (e: React.FormEvent) => {
    ...
    if (updatedCustomer) {
        setSelectedCustomer(updatedCustomer); // Update UI Balance immediately
        // Refresh transactions list
        const loans = await db.loans.getByCustomerId(updatedCustomer.id);
        const payments = await db.payments.getByCustomerId(updatedCustomer.id);
         const all = [
            ...loans.map(l => ({ ...l, type: 'LOAN' })), 
            ...payments.map(p => ({ ...p, type: 'PAYMENT' }))
        ].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                                ...
                                <tbody>
                                    {transactions.map((t: any) => (
                                        <tr key={t.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                            <td className="px-4 py-3 font-mono">{new Date(t.date).toLocaleDateString()}</td>
