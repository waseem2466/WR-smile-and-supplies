...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setStats({
        todaySales: todayBills.reduce((sum, b) => sum + b.finalAmount, 0),
        monthSales: allBills.reduce((sum, b) => sum + b.finalAmount, 0), // Simplified for demo
              ...
              <tbody>
                {bills.slice(0, 5).map(bill => (
                  <tr key={bill.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                    <td className="px-4 py-3">{bill.number}</td>
                    <td className="px-4 py-3">{bill.customerName}</td>
                    <td className="px-4 py-3 text-right">LKR {bill.finalAmount.toLocaleString()}</td>
