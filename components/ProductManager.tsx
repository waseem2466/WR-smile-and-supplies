...
export const ProductManager: React.FC = () => {
  ...
  const calculatePricing = (data: Partial<Product>) => {
    ...
    return { totalCost, sellingPrice: Math.ceil(selling) }; // Round up for cleaner prices
  };
            ...
            <div className="p-4 bg-white/5 rounded-xl border border-white/10 space-y-3">
              ...
              />
               <div className="flex justify-between items-center text-sm pt-2 border-t border-white/10">
                ...
                <span className="text-green-400 font-mono font-bold text-lg">LKR {formData.sellingPrice?.toFixed(2) || '0.00'}</span>
              </div>
            </div>
              ...
              <tbody>
                {products.map(p => (
                  <tr key={p.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                    ...
                    <td className="px-4 py-3 text-right font-mono font-bold text-green-400">{p.sellingPrice.toLocaleString()}</td>
